...
public class TabuleiroSecreto {
    public static void main(String[] args) throws IOException {
        ...
        int tamTabuleiro = Integer.parseInt(nq[0]);
        int numOperacao = Integer.parseInt(nq[1]);
        int[][] matriz = new int[tamTabuleiro][tamTabuleiro];
        ...
        for (int i = 0; i < numOperacao; i++) {
            ...
            int tipoOperacao = Integer.parseInt(operacao[0]);
    ...
    private static void imprimirFrequenteColunaX(int coluna, int[][] matriz) {
        Map<Integer, Integer> map = new HashMap<>();
        map.put(matriz[0][coluna - 1], 1);

        for (int i = 1; i < matriz.length; i++) {
            if (map.containsKey(matriz[i][coluna - 1])) {
                map.put(matriz[i][coluna - 1], map.get(matriz[i][coluna - 1]) + 1);
            } else {
                map.put(matriz[i][coluna - 1], 1);
            }
        }

        var key = map.entrySet().stream().max((entry1, entry2) -> entry1.getValue() > entry2.getValue() ? 1 : -1).get().getKey();
        if (map.get(key) == 1) {
            key = map.entrySet().stream().max((entry1, entry2) -> entry1.getValue() > entry2.getValue() ? 1 : -1).get().getKey();
        }
    ...
    private static void atribuiColunaX(int coluna, int valor, int[][] matriz) {
